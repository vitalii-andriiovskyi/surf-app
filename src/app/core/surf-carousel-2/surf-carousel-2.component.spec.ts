...
@Component({
  template: `
    <div class="carousel-wrapper" style="width: 1000px;">
      <surf-carousel-2 [cycled]="true" [slidersQuantity]="{'0': 1, '480': 2, '800': 3, '992': 4}">
...
describe('SurfCarousel2Component with prop "cycled=true"', () => {
  ...
  let carouselComponent: SurfCarousel2Component;
  ...
  beforeEach(() => {
    ...
    deCarouselComponent = fixtureHost.debugElement.query(By.css('surf-carousel-2'));
    fixtureHost.detectChanges();

    deNavButtons = fixtureHost.debugElement.queryAll(By.css('.surf-carousel-2-nav > div'));
    leftButton = deNavButtons[0].nativeElement;
    rightButton = deNavButtons[1].nativeElement;
    deStages = fixtureHost.debugElement.queryAll(By.css('.surf-carousel-2-stage'));
    deSlides = fixtureHost.debugElement.queryAll(By.css('.surf-carousel-2-slide-wrapper'));
  });
  ...
  it('should move carousel left  on 5 slides after clicking right button 5 times', () => {
    ...
    deSlides = deStages[0].queryAll(By.css('.active'));
    expect(deSlides.length).toBe(0, '1t stage shouldn\'t have sliders with .active class');
    deSlides = deStages[1].queryAll(By.css('.surf-carousel-2-slide-wrapper'));
    expect(deSlides[0].nativeElement.classList.contains('active')).toBeTruthy('first slide of 2d stage must have .active class');

    deNavButtons[1].triggerEventHandler('click', null);
    fixtureHost.detectChanges();
    expect(deStages[0].nativeElement.style.transform).toBe('translateX(1000px)', '1t stage is after second when last slide if 2d stage has .active');
  });
  ...
  it('should move carousel right on 1 slide after clicking left button', () => {
    ...
    deSlides = deStages[0].queryAll(By.css('.surf-carousel-2-slide-wrapper'));
    expect(deSlides[3].nativeElement.classList.contains('active')).toBeFalsy('4th slide if 1t stage doesn\'t have .active');
    deSlides = deStages[1].queryAll(By.css('.surf-carousel-2-slide-wrapper'));
    expect(deSlides[4].nativeElement.classList.contains('active')).toBeTruthy('last slide if 2t stage has .active');
  });
});
...
describe('SurfCarousel2Component without prop "cycled=true"', () => {
  ...
  beforeEach(() => {
    ...
    leftButton = deNavButtons[0].nativeElement;
    rightButton = deNavButtons[1].nativeElement;
    deStage = fixtureHost.debugElement.query(By.css('.surf-carousel-2-stage'));
    deSlides = fixtureHost.debugElement.queryAll(By.css('.surf-carousel-2-slide-wrapper'));
  });
  ...
  it('should move carousel left on 1 slide after clicking right button', () => {
    ...
    expect(deSlides[0].nativeElement.classList.contains('active')).toBeFalsy('1t slide hasn\'t .active class');
    expect(deSlides[1].nativeElement.classList.contains('active')).toBeTruthy('2d slide has .active class');
    expect(leftButton.classList.contains('disabled')).toBeFalsy('left arrow hasn\'t .disable class');
  });
  ...
  it('shouldn\'t move carousel right after clicking left button while 1t slide is active', () => {
    ...
    fixtureHost.detectChanges();
    deSlides = deStage.queryAll(By.css('.surf-carousel-2-slide-wrapper'));
    expect(deSlides[0].nativeElement.classList.contains('active')).toBeTruthy('1t slide hasn .active class');
    expect(leftButton.classList.contains('disabled')).toBeTruthy('left arrow has .disable class');
  ...
  it('should right button be disabled while last slide is active', () => {
    ...
    deSlides = deStage.queryAll(By.css('.surf-carousel-2-slide-wrapper'));
    expect(rightButton.classList.contains('disabled')).toBeTruthy('right arrow has .disable class');
    expect(deSlides[4].nativeElement.classList.contains('active')).toBeTruthy('last slide has .active class');
  });
